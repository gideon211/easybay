...
def test_svg_converter_emits_vector_paths_instead_of_embedded_raster():
    ...
    svg = SvgConverter().convert(_png_bytes(image), mode="vector")
...
def test_svg_fidelity_mode_preserves_exact_rgba_pixels():
    image = Image.new("RGBA", (3, 2))
    image.putdata([
        (1, 2, 3, 255), (17, 33, 65, 128), (250, 249, 248, 0),
        (4, 8, 16, 255), (32, 64, 128, 200), (255, 127, 0, 255),
    ])

    svg = SvgConverter().convert(_png_bytes(image))
    encoded = svg.split("base64,", 1)[1].split('"', 1)[0]
    decoded = Image.open(io.BytesIO(__import__("base64").b64decode(encoded))).convert("RGBA")
